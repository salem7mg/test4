"""Subclass of wxApplicationF.rame, which is generated by wxFormBuilder."""
...
class MainWindow( wxApplicationProject.wxApplicationFrame ):
    ...
    def m_slider1ValueChange(self, event):
        ...
        if(self.isOpenPickleData == False):
            ...
            for i in range(self.mpoint):
                self.axes.add_patch(plt.Rectangle((self.stpoint_x[i], self.stpoint_y[i]), self.bsize, self.bsize, alpha=0.4,
                                                edgecolor="red"))
    ...
    def m_OnOpenImg(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.openImgFolder(event)           

    def m_OnOpenMovie(self, event):
        dialog = wx.FileDialog(None, u'ファイルを選択してください', "", "", "*.mp4")
        if dialog.ShowModal() == wx.ID_OK:
            path = os.path.dirname(dialog.GetPath())
            fileName = path+'\\'+dialog.Filename
            
            dest = fileName+".dir"
            cmd = 'mkdir '+'"'+dest+'"'
            returncode = subprocess.call(cmd, shell=True)
            if(returncode==0 or returncode==1): #新規作成か、すでに存在している場合
                s = fileName.replace(os.sep, '/')
                d = dest.replace(os.sep, '/')
                cmd = '%GSTREAMER_1_0_ROOT_X86_64%\\bin\\gst-launch-1.0.exe filesrc location='+'"'+s+'"'+' ! decodebin ! videoconvert ! queue ! pngenc ! multifilesink location='+'"'+d+'/%08d.png'+'"'
                returncode = subprocess.call(cmd, shell=True)
                if(returncode==0):
                    self.dirpath = dest
                    self.openImgFolder(event) 
    ...
    def OnTimer(self, event):
        if(self.moviePlay == True):
            ...
            frame = self.m_slider1.GetValue() - 1
            if(self.isOpenMovie == True):
                retval = self.capMov.set( 1, frame) # CV_CAP_PROP_POS_FRAMES=1 指定フレームのセット
                retval = self.movFlag, self.movFrame = self.capMov.read()
                self.im = cv2.cvtColor(
                        self.movFrame, cv2.COLOR_BGR2RGB)  # matplot形式
            else:
                cv2img = cv2.imread(self.configdata.image_filelist[frame])
