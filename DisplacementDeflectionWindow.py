...
###########################################################################
## Class DisplacementDeflectionWindow
###########################################################################

class DisplacementDeflectionWindow ( wx.Frame ):
    def __init__( self, parent ):
        wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = u"変位たわみ分布", pos = wx.DefaultPosition, size = wx.Size( 980, 600 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
...
#        frame = self.m_sliderDD.GetValue()
        ...
        self.y2 = (self.data[frame, :, 1] * self.scale) + (self.im.shape[0] / 2)
...
#                                                 edgecolor="red"))
        ...
        self.dammy1_axes.plot(self.pxpy[:, 0] + int(self.bsize / 2), self.y2, color='red')
        self.dammy1_axes.imshow(self.im)
        self.dammy1_canvas.draw()

    def m_replotsliderDD(self, event):
        import configdata
        self.mpoint = configdata.dis.shape[1]
        self.scale = 3000  # 仮設定(表示倍率）
        self.data = configdata.dis.sel(layer=0, features=["dis_x", "dis_y"], stats="mu").values
