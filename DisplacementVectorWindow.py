...
###########################################################################
## Class DisplacementVectorWindow
###########################################################################

class DisplacementVectorWindow ( wx.Frame ):
    def __init__( self, parent ):
        wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = u"変位ベクトル表示", pos = wx.DefaultPosition, size = wx.Size( 980, 600 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
        ...
        self.m_sliderDD = wx.Slider(self, wx.ID_ANY, 0, 0, self.data.shape[0]-1, wx.DefaultPosition, wx.Size(830, 30), style=wx.SL_LABELS | wx.SL_AUTOTICKS)
        ...
        self.Bind(wx.EVT_SLIDER, self.m_replotsliderDV)
        ...
        self.dammy1_axes.set_title('Displacement Vector')
        self.dammy1_axes.imshow(self.im)
        self.dammy1_axes.quiver(self.pxpy[:, 0], self.pxpy[:, 1], self.data[frame, :, 0] * self.scale, self.data[frame, :, 1] * self.scale,
                                angles='xy', width=0.005,color='red',scale=self.scale)
        self.dammy1_canvas.draw()

    def m_replotsliderDV(self, event):
        import configdata
        self.mpoint = configdata.dis.shape[1]
        self.scale = 3000  # 仮設定(表示倍率）
        self.data = configdata.dis.sel(layer=0, features=["dis_x", "dis_y"], stats="mu").values
